...
class ProductStatisticService(
    ...
    fun increaseDailyPopularProduct(command: PopularProductCommand.IncreaseDaily) {
        command.productIdToQuantity.forEach { (productId, quantity) ->
            productStatisticRepository.increaseDailyPopularProduct(productId, quantity.toDouble())
        }
    }
}